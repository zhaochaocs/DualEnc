...
class Trainer(object):
    ...
    def validate(self, valid_iter):
        ...
        for batch in valid_iter:
            ...
            if self.data_type == 'gcn':
                ...
                if isinstance(self.model, NMTModelGCN_DGL):
                    if hasattr(batch, 'morph'):
                        outputs, attns, dec_state = self.model(src, tgt, dgl_batched_graph, src_lengths, morph, mask_morph)
                    else:
                        outputs, attns, dec_state = \
                            self.model(src, tgt, dgl_batched_graph, src_lengths, src2=src2, lengths2=src_lengths2)
                    ...
                    if hasattr(batch, 'morph'):
                        ...
                        outputs, attns, dec_state = \
                            self.model(src, tgt, src_lengths,
                                       adj_arc_in, adj_arc_out, adj_lab_in,
                                       adj_lab_out, mask_in, mask_out,
                                       mask_loop, mask_sent, src2=src2, lengths2=src_lengths2)

            else:
                outputs, attns, _ = self.model(src, tgt, src_lengths, None, src2, src_lengths2)
    ...
    def drop_checkpoint(self, opt, epoch, fields, valid_stats):
        ...
        filePath = '%s_acc_%.2f_ppl_%.2f_e%d.pt' % (opt.save_model, valid_stats.accuracy(), valid_stats.ppl(), epoch)
        if os.path.exists(filePath):
            os.remove(filePath)
        torch.save(checkpoint, filePath)
        # with open("data/trained_model.txt", 'w') as fw:
        #     fw.write('{}\n'.format(filePath))
        with open("{}.txt".format(opt.save_model), 'w') as fw:
            fw.write('{}\n'.format(filePath))
    ...
    def _gradient_accumulation(self, true_batchs, total_stats,
        ...
        for batch in true_batchs:
            ...
            if self.data_type == 'text':
                ...
                _, src_lengths = batch.src
                report_stats.n_src_words += src_lengths.sum()
                adj_arc_in, adj_arc_out, adj_lab_in, adj_lab_out, mask_in, mask_out, mask_loop, mask_sent = onmt.io.get_adj(batch)
                dgl_batched_graph = onmt.io.get_dgl_batched_graph(batch)
                if hasattr(batch, 'morph'):
                #     doStuff(a.property)
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                if self.grad_accum_count == 1:
                    self.model.zero_grad()
                if self.data_type == 'gcn':
                    if isinstance(self.model, NMTModelGCN):
                        if hasattr(batch, 'morph'):
                            outputs, attns, dec_state = self.model(src, tgt, src_lengths,
                                       adj_arc_in, adj_arc_out, adj_lab_in,
                                       adj_lab_out, mask_in, mask_out,
                    ...
                    outputs, attns, dec_state = \
                        self.model(src, tgt, src_lengths, dec_state, src2, src_lengths2)
                    # outputs: Tt*B*E    attens:{std(2)/copy(2): Tt*B*Ts}
