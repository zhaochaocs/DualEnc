...
class CopyGenerator(nn.Module):
    ...
    * :math:`p_{copy}` the probility of copying a word instead.
    ...
    def __init__(self, input_size, tgt_dict):
        ...
        self.agent_idxs = []
        for idx, token in enumerate(self.tgt_dict.itos):
            if re.match(r"(agent|patient|bridge)-\d", token, flags=0):
                print('find {} from the target {}...'.format(token, idx))
                self.agent_idxs.append(idx)
...
class CopyGeneratorLossCompute(onmt.Loss.LossComputeBase):
    ...
    def _compute_loss(self, batch, output, target, copy_attn, align):
        ...
        scores = self.generator(self._bottle(output),   # Tt * B * E
                                self._bottle(copy_attn),    # Tt * B * Ts
                                batch.src_map)      # Ts * B * V_src_size
        loss = self.criterion(scores, align, target)
        scores_data = scores.data.clone()       #B*T, |V|+|V-copy|
        ...
        scores_data = self._bottle(scores_data)     # Tt*B, |V|+|V-copy|
