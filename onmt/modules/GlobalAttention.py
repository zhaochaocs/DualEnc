...
class GlobalAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    def __init__(self, dim, coverage=False, attn_type="dot"):
        ...
        self.linear_out2 = nn.Linear(dim * 3, dim, bias=out_bias)
    ...
    def score(self, h_t, h_s):
        """
        Args:
          h_t (`FloatTensor`): sequence of queries `[batch x tgt_len x dim]`    64,1,256
          h_s (`FloatTensor`): sequence of sources `[batch x src_len x dim]`    64,10,256
    ...
    def forward(self, input, memory_bank, memory_lengths=None, coverage=None):
        ...
        if isinstance(memory_bank, tuple):
            return self.forward2(input, memory_bank, memory_lengths, coverage)
        ...
        attn_h = self.linear_out(concat_c).view(batch, targetL, dim)        # decoding output
        if self.attn_type in ["general", "dot"]:
