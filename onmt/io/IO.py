...
def make_features(batch, side, data_type='text'):
    ...
    Returns:
        ...
        of size (len x batch).
    """
    assert side in ['src', 'src2', 'tgt']
    if isinstance(batch.__dict__[side], tuple):     # toke_id and token_len
...
def get_adj(batch):
    ...
    sent_mask = torch.lt(torch.eq(batch.src[0].data, 1), 1) # if there is a sent, then mask=1
        ...
        for a, arc in enumerate(de):
            ...
            if arc_0 == '<unk>' or arc_0 == '<pad>':
                ...
                if arc_0 == "A0":
                    ...
                    raise ValueError("Do not support the edge type {}".format(arc_0))
...
def build_dataset(fields, data_type, src_path, src_path2, tgt_path, src_dir=None,
    ...
    if not src_path2 == "":
        src_examples_iter2, num_src_feats2 = \
            _make_examples_nfeats_tpl(data_type + "_src2", src_path2, src_dir,
                                      src_seq_length_trunc, sample_rate,
                                      window_size, window_stride,
                                      window, normalize_audio)
    ...
    if data_type == 'text':
        dataset = TextDataset(fields, src_examples_iter, tgt_examples_iter, src_examples_iter2,
...
def build_dataset_gcn(fields, data_type, src_path, src_path2, tgt_path,
    ...
    dataset = GCNDataset(
                ...
                node2_examples_iter, morph_examples_iter, src_examples_iter2,
                num_src_feats, num_tgt_feats,
                src_seq_length=src_seq_length,
                tgt_seq_length=tgt_seq_length,
                dynamic_dict=dynamic_dict,
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    if data_type == 'text':
        ...
        _build_field_vocab(fields["src2"], counter["src2"],
                           max_size=src_vocab_size,
                           min_freq=0)
        print(" * src2 vocab size: %d." % len(fields["src2"].vocab))
...
def _make_examples_nfeats_tpl(data_type, src_path, src_dir,
    ...
    if data_type.startswith('text_'):
        src_examples_iter, num_src_feats = \
            TextDataset.make_text_examples_nfeats_tpl(
                src_path, src_seq_length_trunc, data_type.split("_", 1)[1])
