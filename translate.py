...
import onmt.modules
import opts
from onmt.translate.Beam import select_beam
...
def main():
    ...
    if opt.data_type == 'gcn':
        data = onmt.io.build_dataset_gcn(fields, opt.data_type,
                                 opt.src, opt.src2, opt.tgt, opt.src_label,
    ...
    realization = out_candidates
    with codecs.open(opt.output, 'w', 'utf-8') as out_file:
        out_file.write('\n'.join(realization))
        out_file.write('\n')
