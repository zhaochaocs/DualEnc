...
# opt.brnn = (opt.encoder_type == "brnn") or (opt.encoder2_type == "brnn")
if opt.seed > 0:
...
def make_dataset_iter(datasets, fields, opt, is_train=True):
    ...
    device = torch.device(f'cuda:{opt.gpuid[0]}') if opt.gpuid else -1
...
def train_model(model, fields, optim, data_type, model_opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if epoch >= opt.start_checkpoint_at and valid_stats.ppl() < valid_ppl:
...
def main():
    ...
    data_type = first_dataset.data_type     # data_type: GCN
    ...
    # Build model.
    # make encoder: load embedding based on the vocab dict
    # make decoder
