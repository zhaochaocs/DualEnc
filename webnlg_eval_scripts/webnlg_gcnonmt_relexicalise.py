...
def main(argv):
    ...
    lowercased = False
    for opt, arg in opts:
        if opt in ('-i', '--inputdir'):     # prediction input file
            inputdir = arg
            input_data = True
        elif opt in ('-f', '--filedir'):    # prediction output file
            filepath = arg
            input_filepath = True
    ...
    if partition:
        if category=='seen':
            ...
            input_files(inputdir, filepath, relex=True, parts=[partition], doCategory=UNSEEN_CATEGORIES, fileid=gpuid, lowercased=lowercased)
        else:
            input_files(inputdir, filepath, relex=True, parts=[partition], fileid=gpuid, lowercased=lowercased)
    else:
        input_files(inputdir, filepath, relex=True, parts=['train', 'dev'])
